        ...
        result.append(
            {
                ...
                "size": fs_capacity_info[1],
                "used": fs_capacity_info[2],
                "free": fs_capacity_info[3],
                "used%": int(fs_capacity_info[4][:-1]),
...
def check_evaluate(source):
    result = []
    for fs in source:
        result.append(evaluate_rules(fs, RULES, key=lambda x: x["used%"]))
